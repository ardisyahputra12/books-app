...
class CReturnBook extends HTMLElement {
  connectedCallback() {
    this.render();
  }
  ...
  render() {
    this.innerHTML = `
      <div class="w-full h-full fixed inset-0 z-50 bg-black backdrop-blur-md bg-opacity-40">
        <div class="h-3/5 w-11/12 md:w-9/12 lg:w-1/2 px-4 md:px-6 py-6 m-6 mx-auto flex flex-col justify-between shadow-[0_3px_8px_0_rgba(0,0,0,0.24)] rounded-lg bg-white">
          <h3 class="text-black text-2xl font-bold mb-6 text-center" tabindex="0">Return Book</h3>
          <div>
    ...
    btnSubmit.addEventListener('click', async () => {
      ...
      if (bookTitle && borrower) {
        const book = await AppSource.returnBook(borrower, bookTitle);
...
customElements.define('c-return-book', CReturnBook);
